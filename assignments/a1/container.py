...
class PriorityQueue(Container):
    ...
    def add(self, item):
        ...
        if len(self._queue) == 0:
            ...
            if self._less_than(item, self._queue[-1]):
                self._queue.append(item)
            elif self._less_than(self._queue[0], item):
                self._queue = [item] + self._queue
            else:
                for itr in self._queue:
                    if not self._less_than(item, itr):
                        self._queue.insert(self._queue.index(itr), item)
                        break
